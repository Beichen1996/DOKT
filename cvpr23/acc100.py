...
cifar10_unlabeled   = CIFAR100('../data', train=True, download=True, transform=test_transform)
...
def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, epoch_loss, vis, plot_data):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 4:
            ...
            if bacc < acc:
                bacc = acc 
            print('Val Acc: {:.3f} \t   Loss: {:.3f}'.format(acc, losss))
    print('>> Finished.')
    return bacc
...
if __name__ == '__main__':
    ...
    for trial in range(0,10):
        ...
        name =  './cifar100/' + str(version)+'/100_' + str(1000*(trial+1)) + '.npy'
        print (name)
        ...
        #acc,accclass = test(models, dataloaders, mode='test')
